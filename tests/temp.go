...
func f1() {
	...
	//	s = "map"
	//	//fmt.Println(string('a' + ('z' - 'z' + 2) - 1))
	////	return
	//	for i := 0; i < len(s); i++ {
	//		if s[i] >= 'a' && s[i] <= 'z' {
	//			if s[i] + 2 <= 'z' {
	//				fmt.Print(string(s[i] + 2))
	//			} else {
	//				//fmt.Print("ori: ", string(s[i]))
	//				fmt.Print(string('a' + (s[i] + 2 - 'z') - 1))
	//			}
	//		} else {
...
PBuijeoTSpsVLaOGuLVjMZXkBvVXwUuHfBihziiavGSYofPNeKsTXruMUumRRPQJzvSzJkKbtSipiqBd`
	...
	for i := 3; i < len(s)-3; i ++ {
		if s[i] >= 'a' && s[i] <= 'z' {
			if isUpper(s[i-1]) && isUpper(s[i-2]) && isUpper(s[i-3]) && isUpper(s[i+1]) && isUpper(s[i+2]) && isUpper(s[i+3]) && !isUpper(s[i-4]) && !isUpper(s[i+4]) {
				fmt.Print(string(s[i]))
			}
		}
	}
